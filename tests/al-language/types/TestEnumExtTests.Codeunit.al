...
// Scope: in-scope
// Fixtures used: EEM Status (60880), EEM Status Ext (60881), EEM Action Queue (60882),
//                EEM Action impls (60883-60885), EEM Action Type (60886), EEM Action Type Ext (60887)
//
// Migrated from AL Runner tests/runner-extras/enum-extension-metadata (EemTests.Codeunit.al).
// These assert plain AL enum/enumextension semantics -- Ordinals()/Names() merging an
// extension's values, base values staying reachable, a no-extension enum matching its
// declaration exactly -- so they belong here, where a real service tier adjudicates them.
...
codeunit 60888 "EEM Tests"
{
    ...
    begin
        ...
        Assert.AreEqual('Blank', NameValue, 'Index 1 must still be the base value Blank');
        Names.Get(2, NameValue);
        Assert.AreEqual('Second value', NameValue, 'Index 2 must still be the base value Second value');
    end;
