...
codeunit 60424 "Test Page Control Fld Defaults"
{
    ...
    [Test]
    procedure Record_PageControlField_UndeclaredEditable_IsEmpty()
    var
        PageControlField: Record "Page Control Field";
    begin
        // [GIVEN] the same control, declaring no Editable property.
        FindBareControl(PageControlField);

        // [THEN] Editable is EMPTY, not 'true' — the asymmetry with Enabled and Visible
        // above is the point of this test. A provider that substitutes the same default for
        // all three text columns passes the two tests above and fails this one.
        Assert.AreEqual('', PageControlField.Editable, 'Editable for a control declaring no Editable property. Unlike Enabled and Visible, an undeclared Editable is expected to report nothing rather than the editable default.');
    end;
