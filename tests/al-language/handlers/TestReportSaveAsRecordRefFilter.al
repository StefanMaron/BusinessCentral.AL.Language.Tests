...
// Scope: in-scope
// Fixtures used: SaveAs RecordRef Row (60577), SaveAs RecordRef Document (60578)
//
...
// Report.SaveAs(id, params, format, stream, recordRef) over a RecordRef whose record
...
codeunit 60579 "Test Report SaveAs RecRef Flt"
{
    Subtype = Test;
    ...
    var
        Row: Record "SaveAs RecordRef Row";
    begin
        Row.DeleteAll();
        Row.Init();
        Row."No." := 'DOC-1';
    ...
    [Test]
    procedure TestReport_SaveAs_FilteredRecordRef_LimitsDatasetToThatRecord()
    var
        Row: Record "SaveAs RecordRef Row";
        RowRef: RecordRef;
    ...
    begin
        ...
        Ok := Report.SaveAs(Report::"SaveAs RecordRef Document", '', ReportFormat::Xml, DatasetOutStream, RowRef);
        if not Ok then
    ...
    [Test]
    procedure TestReport_SaveAs_NoRecordRef_LeavesDataItemUnfiltered()
    var
        TempBlob: Codeunit "Temp Blob";
        DatasetOutStream: OutStream;
    begin
        ...
        if Report.SaveAs(Report::"SaveAs RecordRef Document", '', ReportFormat::Xml, DatasetOutStream) then
