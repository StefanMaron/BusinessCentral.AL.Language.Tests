...
//
// Field.SetValue(x) on a page is a VALIDATE, not an assignment — that is what makes a card
// fill in a caption when you pick an id, and what makes it refuse a value that breaks an
// invariant.
//
...
codeunit 60713 "Test Page OnValidate Tests"
{
    Subtype = Test;
    ...
    var
        Row: Record "Test Page OnValidate Row";
    begin
        Row.DeleteAll();
    end;
    ...
    var
        Row: Record "Test Page OnValidate Row";
        Card: TestPage "Test Page OnValidate Card";
    begin
        Initialize();
        Seed('V-1');
